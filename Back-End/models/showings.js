...
Showings.findByAllShowing = (result) => {
  connection.query(`select showings.id AS "showing_id", films.id AS "film_id", showings.showing_screen, showings.showing_time, (screens.screen_max_seats-(SELECT COUNT(*) FROM tickets WHERE ticket_showing = showings.id)) AS "remaining_seats"  from showings 
  join films on showings.showing_film = films.id
